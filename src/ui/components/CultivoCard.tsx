...
export function CultivoCard({ cultivo }: CultivoCardProps) {
  const nombrePlanta = cultivo.tipoPlanta?.nombre ?? "Planta sin identificar";
  ...
  return (
    <article className="bg-slate-900 border border-slate-800 rounded-xl p-4 hover:border-blue-500/50 transition-colors">
      <div className="flex items-center justify-between mb-2">
        <h4 className="text-slate-100 font-semibold">{nombrePlanta}</h4>
        <span className="text-xs font-medium px-2 py-0.5 rounded-full bg-blue-500/10 text-blue-400 border border-blue-500/20">
          {ESTADO_CULTIVO_LABELS[cultivo.estado]}
        </span>
      </div>
      {cultivo.tipoPlanta?.nombreCientifico && (
      ...
      <ul className="space-y-1 text-sm text-slate-300">
        <li>
          ...
          <span className="text-slate-200">
            {cultivo.terreno?.nombre ?? "Sin terreno"}
          </span>
        </li>
        <li>
          Siembra:{" "}
          <span className="text-slate-200">{cultivo.fechaSiembra}</span>
        </li>
      </ul>
    </article>
  )
